...
class Objet:
    ...
    def score_combat_contextuel(self, joueur, carte, Jeu):
        """Score local d'ordre d'utilisation en combat.

        La priorite globale reste le tie-break de draft, mais en combat on prefere
        d'abord les reponses precises/reutilisables avant les consommables generiques.
        """
        score = self.priorite
        reponse_precise = False
        if self.types_tags and any(t in getattr(carte, 'types', ()) for t in self.types_tags):
            score += 120
            reponse_precise = True
        if self.puissance_tags and getattr(carte, 'puissance', None) in self.puissance_tags:
            score += 120
            reponse_precise = True
        if self.actif and not reponse_precise:
            score -= 30
            score -= 2 * self.pv_bonus
        if getattr(self, 'non_combattant', False):
            score -= 100
        if getattr(carte, 'dommages', 0) >= joueur.pv_total:
            score += 20
        return score
