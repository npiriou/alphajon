...
from policies.break_features import extract_break_observation, legal_break_actions, observation_size
from simu import ordonnanceur


class NeedBreakAction(Exception):
    def __init__(self, observation, info):
        super().__init__("break action required")
        self.observation = observation
        self.info = info


class _BreakControlledPolicy:
    ...
    def choose_item_to_break(self, state, legal_actions):
        ...
        obs = extract_break_observation(state["player"], state["game"])
        info = self.env._decision_info(state, legal_actions)
        raise NeedBreakAction(obs, info)
...
class BreakEnv:
    ...
    def _build_game(self):
        ...
        for i, nom in enumerate(noms):
            ...
            joueur.policy = _BreakControlledPolicy(self) if i == self.controlled_seat else HeuristicPolicy("ev")
